...
| **[Kōei (後衛)](specs/zokyo/koei.md)** | *"rearguard"* | Rear dual-arc proximity — a left/right VL53L4CX pair behind a PCA9546 mux widens the rear radar from one beam to a spread arc; `alert` + [Kehai](#kehai-気配--sensed-presence) key off the nearer arc. First **multi-instance sensor**, first to **reshape** an existing contract field (split `distance_mm`, repack the `Distance` char). **Superseded by [Zanshin (残心)](specs/zokyo/zanshin.md)** — the VL53L5CX 8×8 field replaces the two arcs; `distance_l/r_mm` + `alert` persist unchanged. | superseded |
...
| **[Bunshin (分身)](specs/zokyo/bunshin.md)** | *"divided body"* | Two hosts, one perception — a **second QT Py** runs the *identical* firmware and splits the sensor set by what's plugged into each (`fwd` head-pod / `aft` pack-pod), then the glasses and phone **federate both streams at the consumer**. First **multi-host** Zōkyō, first **consumer-side merge** (an authority table arbitrates a channel's source), and first **runtime-configurable** contract behaviour — [Operator](specs/zokyo/bunshin.md#user-selectable-precedence-operator) lets the wearer pick per-channel precedence live. Adds a multi-producer model + a `board` CSV column; the GATT table is untouched (same service on both pods). | active |
| **[Andon (行灯)](specs/zokyo/andon.md)** | *"paper lantern"* | On-body **8×12 LED-matrix surface** (Modulino ABX00152) — the platform's **third output surface** after the NeoPixel and the OLED pane, and the first that is a **spatial pixel field**. Draws a calm falling-raindrop **flair** by default; the panel future sensor-driven faces will paint. First **new-BOM output** device; ships at 0x39 (collides with the APDS9960, so **readdressed to 0x3F**) and is driven by **raw Wire** (no library) to keep the firmware dep-free. No contract change. | active |
| **[Zanshin (残心)](specs/zokyo/zanshin.md)** | *"lingering awareness"* | Rear **depth field** — one **VL53L5CX** 8×8 multizone ToF (on the PCA9546 mux ch0) supersedes Kōei's two VL53L4CX arcs: the rear becomes a 64-zone azimuth+range map, not two beams. Derives `distance_l/r_mm` + `alert` from the field's halves (**CSV + reflex unchanged**) and streams the full field over a **new binary characteristic** → a rear depth panel in both apps (the rear twin of Metsuke). First **multizone rear**; realizes Kōei's forward path. | active |
| **[Enrai (遠雷)](specs/zokyo/enrai.md)** | *"distant thunder"* | Lightning sense — an **AS3935** "Franklin" detector (direct on the bus at 0x03, no mux) reports each strike's distance + relative energy. The firmware **polls** it every loop (no IRQ pin wired), logs a **last-strike snapshot + cumulative count** to the CSV (`lightning_km`/`lightning_energy`/`lightning_strikes`), and notifies a **new string Lightning characteristic event-driven** → a full readout + strike-flash on the phone, a lean nearest-km flash on the glasses. First **event-based** sense (strikes are intermittent, not sampled); brought up live on an overhead storm. | active |
...
| Part | What it does | Alternatives / upgrades |
|------|--------------|-------------------------|
| **VL53L5CX** — 8×8 multizone ToF (I²C 0x29) | 64-zone **depth field**, ~63° FoV, ~4 m, 15 Hz @ 8×8 — the rear sense ([Zanshin](specs/zokyo/zanshin.md)), on the [PCA9546 mux](#host--infrastructure) ch0. Derives `distance_l_mm` / `distance_r_mm` + `alert` from the field's halves and streams the full depth grid. | **VL53L4CX** ×2 (the former [Kōei](specs/zokyo/koei.md) dual-arc — two point beams, no field); **VL53L1X** (cheaper, 4 m); **VL53L4CD** (short-range budget); ultrasonic **HC-SR04** for long/cheap |
...
| **AS3935** — Franklin lightning detector (I²C 0x03) | Detects lightning strikes and estimates distance (1 km = overhead … ~40 km, or out-of-range) + relative energy — the storm sense ([Enrai](specs/zokyo/enrai.md)), direct on the bus (0x03, no mux). **Polled** every loop (no IRQ pin wired); fills `lightning_km` / `lightning_energy` / `lightning_strikes` + the Lightning char. SparkFun Qwiic board. | **DFRobot Gravity AS3935** (same chip); a bare **CJMCU-3935** breakout; wiring the **INT** pin to a GPIO for interrupt-driven (vs polled) capture — the [Enrai](specs/zokyo/enrai.md) forward path |
...
| **Modulino LED Matrix (ABX00152)** — 8×12, 4-bit grayscale (I²C, readdressed to 0x3F) | On-body LED-matrix output *surface* — [Andon](specs/zokyo/andon.md)'s spatial pixel field: a falling-raindrop flair (bright head, dim tail) today, sensor-driven faces later. On-module MCU behind a simple I²C protocol; driven by raw Wire (no library). Ships at 0x39, which collides with the [APDS9960](#sensors), so readdressed to 0x3F on the bench. | The UNO R4 WiFi built-in matrix (same panel); a larger NeoPixel/RGB matrix for colour + brightness; an SPI OLED/TFT for true graphics |
